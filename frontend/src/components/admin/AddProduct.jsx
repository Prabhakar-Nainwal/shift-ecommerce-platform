...
const CATEGORIES = ["Electronics", "Cloths", "Shoes", "Books", "Jewelry", "Home & Kitchen", "Sports", "Beauty", "Toys", "Automotive"];
                ...
                ${coverPreview ? "border-indigo-300" : "border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/40 bg-gray-50"}`}>
                    ...
                    <div className="grid grid-cols-2 gap-x-5 gap-y-5">
                        ...
                        <div>
                            ...
                            <input className={inp} placeholder="e.g. Noise" value={form.brand} onChange={set("brand")} />
                        </div>
                        <div>
                                    ...
                                    <div className="absolute z-20 mt-1 w-full bg-white border border-gray-100 rounded-xl shadow-xl max-h-52 overflow-y-auto">
                                        {CATEGORIES.map(c => (
                                            <button key={c} type="button" onClick={() => { setForm(f => ({ ...f, category: c })); setCatOpen(false); }}
                                                className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${form.category === c ? "bg-indigo-50 text-indigo-600 font-semibold" : "text-gray-700 hover:bg-gray-50"}`}>
                                                {c}
                                            </button>
                                        ))}
                                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input className={inp} type="number" step="0.1" min="0" max="5" placeholder="4.6" value={form.rating} onChange={set("rating")} />
                            </div>
                            <div>
                                <label className={label}>No. of Reviews</label>
                                <input className={inp} type="number" placeholder="342" value={form.numReviews} onChange={set("numReviews")} />
                            </div>
                        </div>
                    </div>
